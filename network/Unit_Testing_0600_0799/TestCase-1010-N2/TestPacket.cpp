...
void CTestPacket::calRate()
{
 	...
 	double TimesOfReceived = (double)(EndTime - m_StartTime) / CLOCKS_PER_SEC;
 	std::cout << "duration time: " << TimesOfReceived << std::endl;
 
 	long double DateSize = (m_TotalPacketOfPast * 8)/(1000 * 1000);
 	std::cout << "Data size: " << DateSize << "Mbit." << std::endl;   
 
 	long double Rate = DateSize / TimesOfReceived;
