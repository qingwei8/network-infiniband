...
int main()
{
	...
	try
	{
		...
		pPacket->setSendQuality(hiveNetworkCommon::LOW_SEND_QUALITY);
		pPacket->setSendMode(hiveNetworkCommon::UNICAST);

		hiveNetworkCommon::CNetworkController::getInstance()->modifyDefaultPacketCode2NetworkNodeMap(pPacket->getPacketSize(), pPacket->getSendQuality(), "IB");
